function pagarServicio() {
  ...
  if (!servicio) {//si no se elige el servicio nos da un mensaje de advertencia
    Swal.fire("Servicio requerido", "Seleccione un tipo de servicio", "warning");
    return;
  }

  if (idServicio === "") {//si no se elige el servicio nos da un mensaje de advertencia
    Swal.fire("ID requerido", "Debe ingresar el ID del servicio", "warning");
    return;
  }

  if (isNaN(monto) || monto <= 0) {//si no se elige el servicio nos da un mensaje de advertencia
    Swal.fire("Monto inválido", "Ingrese un monto mayor a 0", "warning");
    return;
  }

  if (monto > saldo) {
    Swal.fire("Saldo insuficiente", `Tu saldo es $${saldo.toFixed(2)}`, "error"); //acá se valida si el monto a pagar es mayor al saldo actual, entonces nos muestra un error
    return;
  }
  ...
  historial.push({ tipo: `Servicio - ${servicio}`, monto: monto.toFixed(2), fecha, id: idServicio }); //El nombre con el cual se guarda el movimiento y que servicio fue
